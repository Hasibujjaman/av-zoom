...
% ---------------- Plane-wave model (MATLAB convention: 90°=broadside) ----------------
make_multichannel = @(sig, azimuth) ...
    [ apply_delay(sig, micPos(1)*cos(deg2rad(azimuth))/c), ...
      apply_delay(sig, micPos(2)*cos(deg2rad(azimuth))/c) ];
